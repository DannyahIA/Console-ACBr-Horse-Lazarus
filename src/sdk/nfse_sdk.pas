...
var
  ...
  FileName: string;
begin
  //atriubui um nome e local ao arquivo
  FileName := 'd:\temp\meu_teste.xml';

  //caso exista um arquivo ja, o deleta
  deleteFile(FileName);

  //faz o request no servidor nfse com Get
  Response := TRequest.New.BaseURL(GetUrl('nfse/arquivo/xml')).Get;

  //verifica se o status for diferente de 200 (OK)
...
begin
  ...
  if Response.StatusCode <> 200 then
  begin
    //se for diferente de 200 mostra o erro e cai fora
    ShowMessage('Erro ao baixar o arquivo: ' + Response.Content);
    exit;
  end;
